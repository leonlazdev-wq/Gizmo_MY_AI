...
def render_activity(activity):
    ...
    for item in items:
        ...
        parts.append(
            ...
            f"<div class='su-activity-meta'>"
            f"<span>{item.get('level','info').upper()}</span>"
            f"<span>{_relative_time(item.get('timestamp',''))}</span>"
            f"</div><div class='su-activity-msg'>{item.get('message','')}</div></div>"
        )
...
def build_status_card():
    ...
    try:
        ...
        drive_info = (
            f"<tr><td style='padding:4px 8px;color:#888'>📁 Drive</td>"
            f"<td style='padding:4px 8px'>{du.used/1024**3:.1f} / {du.total/1024**3:.1f} GB</td></tr>"
        )
    except Exception:
        pass

    return (
        f"<div class='su-card' style='font-family:monospace'>"
        f"<div class='su-muted' style='margin-bottom:10px'>Updated: {datetime.now().strftime('%H:%M:%S')}</div>"
        f"<table style='width:100%;border-collapse:collapse;color:#e0e0f0'>"
        f"<tr><td style='padding:4px 8px;color:#888'>🧠 RAM</td>"
        f"<td style='padding:4px 8px'><span style='color:{ram_color}'>{ram_bar}</span>"
        f"<span style='margin-left:8px'>{ram_used_gb:.1f} / {ram_total_gb:.1f} GB ({ram_pct:.0f}%)</span></td></tr>"
        f"<tr><td style='padding:4px 8px;color:#888'>⚙️ CPU</td><td style='padding:4px 8px'>{cpu_pct:.0f}%</td></tr>"
        f"<tr><td style='padding:4px 8px;color:#888'>🤖 Model</td><td style='padding:4px 8px'>{model_name}</td></tr>"
        f"<tr><td style='padding:4px 8px;color:#888'>⏱️ Uptime</td><td style='padding:4px 8px'>{_uptime_text()}</td></tr>"
        f"{drive_info}</table></div>"
    )
    ...
    with gr.Tabs():
        with gr.Tab("⚡ Quick Prompts"):
            try:
                ...
                with gr.Group(elem_classes=["su-card"]):
                    gr.Markdown(
                        "<ul class='su-list'>"
                        "<li>Explain this topic like I'm 12.</li>"
                        "<li>Quiz me on this chapter.</li>"
                        "<li>Turn these notes into flashcards.</li>"
                        "<li>Summarize this in 5 bullet points.</li>"
                        "</ul>"
                    )
            except Exception as e:
        ...
        with gr.Tab("💾 Export Chat"):
            try:
                ...
                with gr.Group(elem_classes=["su-card"]):
                    ...
                    export_btn.click(on_export, [activity_state, fmt, filename], [export_status, link, activity_state], show_progress=False)
            except Exception as e:
                        ...
                        with gr.Row():
                            for label, prompt in row:
                                gr.Button(label).click(
                                    lambda a, n=label, p=prompt: on_switch(a, n, p),
                                    [activity_state],
                                    [persona_status, active, fallback_prompt, activity_state],
                                    show_progress=False,
                                )
            except Exception as e:
        ...
        with gr.Tab("📝 Notes"):
            try:
                ...
                with gr.Group(elem_classes=["su-card"]):
                    notes = gr.Textbox(
                        label="Notes (auto-saved to Drive)",
                        lines=10,
                    ...
                    copy_btn.click(
                        None, None, notes_status,
                        js="() => { const t=document.querySelector('#su-notes-box textarea'); if(!t){return '⚠️ Could not copy';} navigator.clipboard.writeText(t.value || ''); return '✅ Copied to clipboard'; }",
                    )
            except Exception as e:
        ...
        with gr.Tab("🔔 Activity"):
            try:
                ...
                with gr.Group(elem_classes=["su-card"]):
                    ...
                    clear_act_btn.click(clear_activity, None, [activity_state, activity_html, unread], show_progress=False)
            except Exception as e:
