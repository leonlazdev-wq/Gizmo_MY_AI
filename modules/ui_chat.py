...
''', elem_id='connector-plus-html')
                ...
                with gr.Row(elem_id='adaptive-toolbar-row', visible=True):
                    # Visual mock: ✂️ 📝 🐞 📎 compact contextual toolbar
                    shared.gradio['adaptive_summarize'] = gr.Button('✂️ Summarize', elem_classes=['refresh-button'])
                    shared.gradio['adaptive_actions'] = gr.Button('📝 Action items', elem_classes=['refresh-button'])
                    shared.gradio['adaptive_bugs'] = gr.Button('🐞 Find bugs', elem_classes=['refresh-button'])
                    shared.gradio['adaptive_task'] = gr.Button('📎 Create task', elem_classes=['refresh-button'])
                    shared.gradio['provenance_btn'] = gr.Button('🕒 Provenance', elem_classes=['refresh-button'])
                shared.gradio['adaptive_output'] = gr.Textbox(label='Toolbar output', lines=4, interactive=False)
