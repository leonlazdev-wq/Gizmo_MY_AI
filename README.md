...
# Optional: if this folder is a git clone, update first
# git pull
...
Alternative: run `./start_fedora.sh --share` to auto-create venv, install Fedora deps, apply Pillow compatibility guard, install Python deps, and launch. By default it uses GPU acceleration when available (use `--cpu-only` to force CPU).
